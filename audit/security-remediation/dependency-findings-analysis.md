...
| Critical node-tar | tar | vulnerable via @expo/cli → cacache (tar@6.2.1) | needs tar≥7.5.22 **or** Expo SDK with tar-7-aware CLI | transitive | **build** (Expo CLI), not app runtime | expo@51 → @expo/cli | App does not extract user-controlled TAR archives | **Do not** override to tar@7 on Expo 51 (breaks prebuild `extract`) — SDK upgrade | High if forced override (CI break); Medium for SDK upgrade |
