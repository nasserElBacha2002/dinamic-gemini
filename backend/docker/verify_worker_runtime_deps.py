"""Image build-time verification of ODBC / pyzbar / tesseract (worker image)."""
...
if not ({"spa", "eng"} <= _langs):
    raise SystemExit(
        f"Build verification failed: tesseract languages must include spa+eng. Got: {sorted(_langs)}"
    )
