...
- **DEV (OpenCloud):** migrations run automatically on the Ubuntu server during SSH deploy (`backend/scripts/dev_deploy_db_migrate.sh` after `docker-compose up -d`) — see `docs/deployment/DEV-OPENCLOUD.md`.
