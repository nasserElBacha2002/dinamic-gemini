...
class MinifiedProduct(BaseModel):
    ...
    b: str | None = Field(None, description="Marca del producto (opcional).")
    n: str = Field(..., description="Nombre o descripción del producto.")
    r: str = Field(
        ...,
        description="Razonamiento matemático: e.g., 'Base 3x4=12. 2 layers=24. Top layer missing 3. 24-3=21'",
    )
...
class EntityV21(BaseModel):
    ...
    position_barcode: str | None = Field(
        None,
    ...
    position_label_bbox: list[float] | None = Field(
        None, description="[x1,y1,x2,y2] normalizado 0..1, x1<x2, y1<y2."
    )
    product_label_quantity: int | None = Field(
        None, description="Cantidad en etiqueta de producto."
    )
    product_label_bbox: list[float] | None = Field(
        None, description="[x1,y1,x2,y2] normalizado 0..1."
    )
    ...
    source_image_id: str | None = Field(
        None,
        description="Legacy compatibility only; prefer manifest_entry_id when available.",
    )
...
class FrameRef(BaseModel):
    ...
    width: int | None = Field(default=None, gt=0, description="Ancho del frame en píxeles.")
    height: int | None = Field(default=None, gt=0, description="Alto del frame en píxeles.")
...
class ProductEstimate(BaseModel):
    ...
    brand: str | None = Field(default=None, description="Marca del producto (opcional).")
...
class FinalResult(BaseModel):
    ...
    processing_summary: dict[str, Any] | None = Field(
        default=None,
        description=(
...
class LLMProductObservation(BaseModel):
    ...
    reasoning: str | None = Field(
        default=None, description="Razonamiento matemático del modelo para llegar al conteo."
    )
...
class ConsolidationStats(BaseModel):
    ...
    n_inliers: int | None = Field(
        default=None, description="Número de observaciones después de filtrar outliers."
    )
    min_est: int | None = Field(default=None, description="Estimación mínima.")
    max_est: int | None = Field(default=None, description="Estimación máxima.")
    median_est: float | None = Field(default=None, description="Mediana de las estimaciones.")
    mad: float | None = Field(
        default=None,
        description="Median Absolute Deviation (desviación absoluta mediana) de todas las observaciones.",
    )
    mad_inliers: float | None = Field(
        default=None, description="MAD de los inliers (después de filtrar outliers)."
    )
    conf_mean: float | None = Field(default=None, description="Confianza media de los inliers.")
...
class ConsolidatedProduct(BaseModel):
    ...
    evidence_frames: int = Field(..., ge=0, description="Número de frames que aportaron evidencia.")
    stats: ConsolidationStats | None = Field(
