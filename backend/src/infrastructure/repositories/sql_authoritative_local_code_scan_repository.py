...
class SqlAuthoritativeLocalCodeScanRepository:
    ...
    def create_authoritative_version(
        ...
        *,
        ...
        """Lock asset rows, CAS supersede, insert new current.

        When bound to a caller's transaction the statements join it and the caller owns the
        commit/rollback; otherwise this method owns a short transaction of its own.
        """
        if self._connection is not None:
            cur = self._connection.cursor()  # type: ignore[attr-defined]
            try:
                self._write_authoritative_version(
                    cur,
        ...
        if saved is None:
            raise RuntimeError(f"authoritative insert missing after write id={new_result.id}")
        return saved

    def _write_authoritative_version(
        self,
        cur,
        *,
        new_result: AuthoritativeLocalCodeScanResult,
        ...
        next_version = int(getattr(max_row, "max_v", None) or 0) + 1
