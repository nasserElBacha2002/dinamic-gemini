...
def _horizontal_overlap(
    ...
    """True if two bounding boxes are horizontally aligned enough to belong to the same column.

    ``a`` is treated as the parent (label) region and ``b`` as the candidate value region
    below it. Accepts either an actual overlap of the ``[left, left + width]`` intervals, or
    (when the intervals don't overlap) a gap between them no larger than half of ``a``'s
    width — this tolerates the small horizontal drift OCR bounding boxes commonly have for
    text stacked directly below a label, while still rejecting a value that actually belongs
    to a visually distant, unrelated column. When either region is unknown, the constraint
    cannot be evaluated and is treated as satisfied.
    """
...
class OcrFieldExtractor:
    ...
    def extract(self, read: InternalOcrReadResult) -> OcrFieldExtraction:
        ...
        if not lines and read.full_text.strip():
            lines = [(ln, None, None, None) for ln in read.full_text.splitlines() if ln.strip()]

        for text, conf, region, _block_num in lines:
            self._extract_labeled_line(text, conf, region, out)

        # Spatial: value on the line immediately below a lone label.
        for idx, (text, conf, region, block_num) in enumerate(lines):
            folded = _fold(text).rstrip(":")
            if folded not in _CODE_ALIASES and folded not in _ARTICLE_ALIASES and folded not in _QTY_ALIASES:
                continue
            if idx + 1 >= len(lines):
                continue
            nxt, nconf, nregion, nblock_num = lines[idx + 1]
            if ":" in nxt:
                continue
            nxt_folded = _fold(nxt).rstrip(":")
            if nxt_folded in _CODE_ALIASES or nxt_folded in _ARTICLE_ALIASES or nxt_folded in _QTY_ALIASES:
                # The "next line" is itself a label (e.g. a second column header row),
                # not a value — never adopt it as the label's answer.
                continue
            if block_num is not None and nblock_num is not None and block_num != nblock_num:
                # Different OCR block (typically a different column) — not spatially "below".
                continue
    ...
    def _lines_from_blocks(
    ...
    ) -> list[tuple[str, float | None, tuple[int, int, int, int] | None, int | None]]:
        if not blocks:
            return []
        ...
        lines: list[tuple[str, float | None, tuple[int, int, int, int] | None, int | None]] = []
        for key in sorted(grouped.keys()):
            ...
            if text:
                for sub in text.splitlines():
                    sub = sub.strip()
                    if sub:
                        lines.append((sub, conf, region, block_num))
        for b in singles:
            for sub in (b.text or "").splitlines():
                sub = sub.strip()
                if sub:
                    lines.append((sub, b.confidence, _region(b), b.block_num))
        return lines
