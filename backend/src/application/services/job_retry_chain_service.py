...
class JobRetryChainService:
    ...
    def build(self, selected: Job, *, aisle_id: str) -> RetryChainView:
        if selected.target_type != "aisle" or selected.target_id != aisle_id:
            # Caller must already have scoped the job; defensive guard.
            raise ValueError("selected job is not scoped to aisle")

        siblings = list(
            self._job_repo.list_jobs_for_target("aisle", aisle_id, limit=500)
        )
        by_id = {j.id: j for j in siblings if j.job_type == selected.job_type}
        if selected.id not in by_id:
            by_id[selected.id] = selected

        root = self._find_root(selected, by_id)
        ordered = self._walk_forward(root, by_id)
        if not ordered:
            ordered = [selected]

        # Detect cycles / corruption: drop duplicates preserving first occurrence.
        seen: set[str] = set()
        deduped: list[Job] = []
        for job in ordered:
            if job.id in seen:
        ...
        current = deduped[-1]
        # Prefer last SUCCEEDED as "current result" when present; else tip of chain.
    ...
    def _find_root(self, job: Job, by_id: dict[str, Job]) -> Job:
        current = job
        visited: set[str] = set()
    ...
    def _walk_forward(self, root: Job, by_id: dict[str, Job]) -> list[Job]:
        ...
        while guard < 100:
            ...
            if not next_jobs:
                break
            # Linear chain expectation: take earliest child; ignore forks.
            cursor = next_jobs[0]
