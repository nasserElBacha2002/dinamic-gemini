...
def save_frame(frame: np.ndarray, output_path: str, quality: int = 85) -> str:
    ...
    if not success:
        raise OSError(f"No se pudo guardar el frame en: {output_path}")
...
def load_frame(image_path: str) -> np.ndarray:
    ...
    if frame is None:
        raise OSError(f"No se pudo cargar la imagen: {image_path}")
