...
def _filter_similar_frames_with_cap(
    ...
    prev_frame_small = (
        cv2.resize(prev_frame, (sample_size, sample_size)) if sample_size else prev_frame
    )
    ...
    for current_frame_ref in frames[1:]:
        ...
        if current_frame is None:
            continue
        current_small = (
            cv2.resize(current_frame, (sample_size, sample_size)) if sample_size else current_frame
        )
...
def filter_similar_frames(
    ...
    try:
        return _filter_similar_frames_with_cap(cap, frames, similarity_threshold, sample_size=None)
    finally:
        cap.release()
