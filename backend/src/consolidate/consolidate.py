...
def consolidate(
    ...
    meta: dict[str, dict[str, list[dict[str, Any]]]] = defaultdict(lambda: defaultdict(list))
    ...
    for pallet_id, prod_map in buckets.items():
        ...
        for product_key, observations in prod_map.items():
            ...
            if mad == 0:
                ...
                k = mad_threshold
                inliers = [(x, c) for (x, c) in observations if abs(x - med) <= k * mad]
                if not inliers:
                    # Fallback: usar todas las observaciones
                    inliers = observations[:]
            ...
            if product_counts:
                ...
                best_product = raw_first if isinstance(raw_first, str) and raw_first else "Unknown"
        ...
        consolidated_products.sort(key=lambda p: (-p.estimated_boxes, p.product.lower()))
