...
from src.api.errors.structured_api_http import (
    ...
    IMAGE_PROCESSING_REPOSITORY_UNAVAILABLE,
    IMAGE_RESULT_LOCK_TIMEOUT,
    ...
    POSITION_MERGE_CONFLICT,
    POSITION_MERGE_STALE_PREVIEW,
    POSITION_MERGE_VALIDATION,
    POSITION_NOT_FOUND,
...
_HTTP_EXCEPTION_DISPATCH: dict[type[BaseException], Callable[[BaseException], HTTPException]] = {
    ...
    SupplierExtractionProfileNotFoundError: _structured_fixed(
        404,
        error_code=SUPPLIER_EXTRACTION_PROFILE_NOT_FOUND,
        detail=HTTP_DETAIL_SUPPLIER_EXTRACTION_PROFILE_NOT_FOUND,
    ),
    SupplierExtractionProfileInvalidConfigurationError: _structured_detail(
        422,
        error_code=SUPPLIER_EXTRACTION_PROFILE_INVALID_CONFIGURATION,
        detail=lambda e: str(e),
    ),
    SupplierExtractionProfileActivationFailedError: _structured_fixed(
        409,
        error_code=SUPPLIER_EXTRACTION_PROFILE_ACTIVATION_FAILED,
        detail=HTTP_DETAIL_SUPPLIER_EXTRACTION_PROFILE_ACTIVATION_FAILED,
    ),
    SupplierExtractionProfileRowVersionConflictError: _structured_fixed(
        409,
    ...
    ImageResultLockTimeoutError: _structured_fixed(
        409,
        error_code=IMAGE_RESULT_LOCK_TIMEOUT,
        detail="No se pudo adquirir el bloqueo de la imagen; reintentá en unos segundos.",
    ),
    ...
    PositionMergeValidationError: _structured_detail(
        422,
        error_code=POSITION_MERGE_VALIDATION,
        detail=lambda e: str(e),
    ),
    PositionMergeConflictError: _structured_detail(
        409,
        error_code=POSITION_MERGE_CONFLICT,
        detail=lambda e: str(e),
    ),
    PositionMergeStalePreviewError: _structured_fixed(
        409,
        error_code=POSITION_MERGE_STALE_PREVIEW,
        detail="Los registros cambiaron desde que se generó la previsualización. Revisá nuevamente la fusión.",
    ),
    ...
    AssetProcessingStateConcurrencyError: _structured_detail(
        409,
        error_code=ASSET_PROCESSING_STATE_CONCURRENCY_CONFLICT,
        detail=lambda e: str(e) or "Asset processing state changed concurrently; retry.",
    ),
    StrategyDisabledError: _structured_detail(
        422,
