...
Internal strategy (CODE_SCAN | INTERNAL_OCR)
  ...
  → eligible failure → EXTERNAL_PROVIDER attempt → normalize → validate → persist or terminal status
```

Never sends the whole aisle. Never reprocesses already-resolved assets.
...
Never eligible: `RESOLVED_INTERNAL`, persistence failures, manual result exists, snapshot/config errors, circuit open, prior successful external attempt.
...
- timeout, max_attempts, concurrency, circuit breaker, quantity_max, recoverable codes

Retries **copy** the snapshot (see `retry_aisle_job`); they do not re-read live env for provider choice.

## Attempts

Internal and external attempts are separate rows (`strategy=CODE_SCAN|INTERNAL_OCR` then `EXTERNAL_PROVIDER`). Estimated cost (when usage is available) is stored on `attempt.extra.estimated_cost`.

## Circuit breaker

Process-local (`provider:model`). Opens on timeouts / rate limits / technical failures — **not** on ambiguous / validation / no-result business outcomes.
...
- Job summary: `fallback_progress` (+ existing `asset_progress`, `identification_execution`)
- Observability workspace shows a minimal fallback counters line
- Full per-image attempt UX is Phase 7

## Security

No API keys, full images, or raw provider payloads in logs. Response evidence uses hashes / normalized fields only.
...
4. Confirm `result_json.fallback_progress` and job summary API fields.
