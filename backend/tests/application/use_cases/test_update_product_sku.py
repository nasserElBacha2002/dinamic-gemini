"""Tests for UpdateProductSkuUseCase — Épica 8."""
...
def test_update_sku_sets_sku_description_and_creates_audit() -> None:
    ...
    use_case = UpdateProductSkuUseCase(
    ...
    assert actions[0].action_type == ReviewActionType.UPDATE_SKU
    assert actions[0].before_json.get("sku") == "OLD-SKU"
    assert actions[0].after_json.get("sku") == "NEW-SKU"


def test_update_sku_empty_sku_raises() -> None:
    now = datetime(2025, 3, 6, 12, 0, 0, tzinfo=timezone.utc)
    inv = Inventory("inv-1", "WH", InventoryStatus.DRAFT, now, now)
    aisle = Aisle("aisle-1", "inv-1", "A01", AisleStatus.CREATED, now, now)
    position = Position(
