"""MarkPositionImageMismatch — traceability-only review outcome."""
...
from src.application.use_cases.positions.mark_position_image_mismatch import MarkPositionImageMismatchUseCase
...
def test_mark_image_mismatch_sets_resolution_without_removing_evidence_pointer() -> None:
    ...
    position = Position(
        ...
        confidence=0.9,
        needs_review=True,
        primary_evidence_id="ev-keep",
        created_at=now,
        updated_at=now,
        detected_summary_json={"internal_code": "SKU-X"},
    )
    ...
    use_case = MarkPositionImageMismatchUseCase(
    ...
    assert updated.review_resolution == PositionReviewResolution.IMAGE_MISMATCH
    assert updated.primary_evidence_id == "ev-keep"
    summary = updated.detected_summary_json
    assert isinstance(summary, dict)
    assert summary.get("internal_code") == "SKU-X"

    actions = review_repo.list_by_position("pos-1")
    assert len(actions) == 1
    assert actions[0].action_type == ReviewActionType.MARK_IMAGE_MISMATCH
    assert actions[0].after_json["review_resolution"] == "image_mismatch"


def test_mark_image_mismatch_not_found_raises() -> None:
    now = datetime(2026, 4, 1, 12, 0, 0, tzinfo=timezone.utc)
    inv = Inventory("inv-1", "WH", InventoryStatus.DRAFT, now, now)
    aisle = Aisle("aisle-1", "inv-1", "A01", AisleStatus.CREATED, now, now)
    inv_repo = StubInventoryRepo(inv)
