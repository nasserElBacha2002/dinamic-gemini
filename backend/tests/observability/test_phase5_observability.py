...
def test_metrics_reject_high_cardinality_labels() -> None:
    ...
    before = reg.series_count("t")
    # Forbidden labels are soft-rejected (no new series; no exception for business callers).
    reg.inc("t", "help", {"job_id": "abc"})
    assert reg.series_count("t") == before
    assert reg.get_counter_value("t", {}) == 0.0
...
def test_http_request_id_and_metrics_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # /health is skipped from HTTP cardinality metrics; endpoint must still render.
