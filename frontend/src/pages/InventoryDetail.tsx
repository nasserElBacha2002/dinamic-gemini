...
import { getInventory, getAisles } from '../api/client';
...
export default function InventoryDetail() {
  ...
  const loadAisles = useCallback(async () => {
    ...
    try {
      ...
      setAislesError(
        typeof err.data?.detail === 'string' ? err.data.detail : err.message || 'Failed to load aisles'
      );
    } finally {
      ...
      .catch((e) => {
        ...
        if (!inventoryLoaded) {
          if (err.status === 404) {
            ...
            setInventoryError(
              typeof err.data?.detail === 'string' ? err.data.detail : err.message || 'Failed to load inventory'
            );
          }
        } else {
          setAislesError(
