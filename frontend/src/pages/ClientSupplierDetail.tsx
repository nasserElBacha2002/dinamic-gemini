...
import LabelRecognitionProfileModule from '../features/clients/components/labelRecognition/LabelRecognitionProfileModule';
...
const TAB_VALUES = ['resumen', 'prompts', 'reconocimiento-etiquetas', 'imagenes'] as const;
type SupplierDetailTab = (typeof TAB_VALUES)[number];

function normalizeSupplierTab(raw: string | null): SupplierDetailTab {
  const v = (raw ?? '').trim().toLowerCase();
  if (v === 'instrucciones-ocr') return 'reconocimiento-etiquetas';
  return v === 'prompts' || v === 'reconocimiento-etiquetas' || v === 'imagenes' ? v : 'resumen';
}
      ...
      {supplierQuery.data ? (
        <Box sx={{ width: '100%' }}>
          ...
          >
            ...
            <Tab value="reconocimiento-etiquetas" label={t('clients.supplier_page.tab_extraction_profile')} />
          ...
          {tab === 'reconocimiento-etiquetas' && supplierQuery.data ? (
            <Box sx={{ display: 'grid', gap: 1.5 }} role="tabpanel">
