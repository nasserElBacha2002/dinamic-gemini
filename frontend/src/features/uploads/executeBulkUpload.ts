...
function newBatchId(): string {
  ...
  return `ub-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}
...
function snapshotFrom(
  ...
  const uploadedBytes =
    uploadedBytesOverride ??
    files.reduce((s, f) => {
      if (f.status === 'completed') return s + f.file.size;
      if (f.status === 'uploading' || f.status === 'processing') {
        return s + Math.round((f.file.size * Math.min(100, f.progress)) / 100);
      }
      return s;
    }, 0);
  const progressPct = totalBytes <= 0 ? 100 : Math.min(100, Math.round((uploadedBytes / totalBytes) * 100));
  return {
    uploadBatchId,
    phase,
    files: files.map((f) => ({ ...f })),
...
export interface ExecuteBulkUploadOptions {
  ...
  /** When set, only these clientIds are (re)uploaded; others keep status. */
...
export async function executeBulkUpload(options: ExecuteBulkUploadOptions): Promise<BulkUploadRunResult> {
  ...
  const notify = (phase: BulkUploadProgressSnapshot['phase'], uploadedBytesOverride?: number) => {
    options.onProgress?.(
      snapshotFrom(uploadBatchId, allFiles, batchesCompleted, batchesTotal, phase, uploadedBytesOverride)
    );
  };

  notify(pending.length ? 'uploading' : allFiles.some((f) => f.status === 'failed') ? 'completed_with_errors' : 'completed');
    ...
    while (true) {
      ...
      await uploadOneBatch({
        ...
        onProgress: (loaded, total) => {
          // Approximate global bytes: completed + fraction of active batch.
          const completedBytes = allFiles
            .filter((f) => f.status === 'completed')
            .reduce((s, f) => s + f.file.size, 0);
          const frac = total > 0 ? loaded / total : 0;
          const activeBytes = Math.round(batch.totalBytes * frac);
          for (const f of batch.files) {
            if (f.status === 'uploading') {
              f.progress = Math.min(99, Math.round(frac * 100));
            }
          }
          notify('uploading', completedBytes + activeBytes);
        },
      });
      batchesCompleted += 1;
      notify('uploading');
