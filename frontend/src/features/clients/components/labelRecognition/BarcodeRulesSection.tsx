...
export default function BarcodeRulesSection({ configuration, onChange }: Props) {
  ...
  return (
    <SectionCard title={t('clients.extraction_profile.section_barcode_rules')} variant="outlined">
      <Stack spacing={1.5}>
        ...
        <Typography variant="subtitle2">{t('clients.extraction_profile.section_validation')}</Typography>
        <Box sx={{ display: 'grid', gap: 1.5, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' } }}>
          ...
          >
            {['ANY', 'NUMERIC', 'ALPHANUMERIC', 'UPPERCASE_ALPHANUMERIC', 'HEX'].map((item) => (
        ...
        <Accordion disableGutters elevation={0} sx={{ border: 1, borderColor: 'divider', borderRadius: 1 }}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography variant="subtitle2">{t('clients.extraction_profile.section_advanced')}</Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Stack spacing={1.5}>
              <FormControlLabel
