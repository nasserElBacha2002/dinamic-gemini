...
describe('LoginPage', () => {
  ...
  it('renders login form with username, password and submit button', () => {
    renderLoginPage();
    expect(screen.getByLabelText(/usuario|username/i, { selector: 'input' })).toBeInTheDocument();
    expect(screen.getByLabelText(/contraseña|password/i, { selector: 'input' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /ingresar|login/i })).toBeInTheDocument();
    expect(screen.getByText(/iniciar sesión|login title/i)).toBeInTheDocument();
  });

  it('disables submit when username or password is blank', () => {
    renderLoginPage();
    const submitBtn = screen.getByRole('button', { name: /ingresar|login/i });
    expect(submitBtn).toBeDisabled();
    fireEvent.change(screen.getByLabelText(/usuario|username/i, { selector: 'input' }), { target: { value: 'a' } });
    expect(submitBtn).toBeDisabled();
    fireEvent.change(screen.getByLabelText(/contraseña|password/i, { selector: 'input' }), { target: { value: 'b' } });
    expect(submitBtn).toBeEnabled();
    fireEvent.change(screen.getByLabelText(/usuario|username/i, { selector: 'input' }), { target: { value: '   ' } });
    expect(submitBtn).toBeDisabled();
  });
  ...
  it('submit sends credentials to login API and shows loading state', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/contraseña|password/i, { selector: 'input' }), { target: { value: 'secret' } });
    fireEvent.click(screen.getByRole('button', { name: /ingresar|login/i }));

    expect(mockLogin).toHaveBeenCalledWith({ username: 'admin', password: 'secret' });
    expect(screen.getByRole('button', { name: /ingresando|signing in/i })).toBeInTheDocument();
  });
  ...
  it('on login success stores token and navigates to dashboard', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/usuario|username/i, { selector: 'input' }), { target: { value: 'admin' } });
    fireEvent.change(screen.getByLabelText(/contraseña|password/i, { selector: 'input' }), { target: { value: 'pass' } });
    fireEvent.click(screen.getByRole('button', { name: /ingresar|login/i }));
  ...
  it('on login failure shows error message', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/usuario|username/i, { selector: 'input' }), { target: { value: 'admin' } });
    fireEvent.change(screen.getByLabelText(/contraseña|password/i, { selector: 'input' }), { target: { value: 'wrong' } });
    fireEvent.click(screen.getByRole('button', { name: /ingresar|login/i }));
