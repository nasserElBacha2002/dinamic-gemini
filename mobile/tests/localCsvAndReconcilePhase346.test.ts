...
describe('Phase 4 local CSV export', () => {
  ...
  it('uses position_snapshot_json when present (not legacy forward-fill)', () => {
    const now = '2026-01-01T00:00:00.000Z';
    const session = {
      id: 'session-1',
      inventory_id: 'inv-1',
    ...
    const snapshot = {
      labelId: 'pos_snap',
      positionLabelId: 'pos_snap',
      displayName: '7 RIGHT N1 01/01',
      canonicalKey: '7|RIGHT|1|1|1',
      pallet: '7',
      side: 'RIGHT',
      level: 1,
      markerIndex: 1,
      markerTotal: 1,
      formattedMarker: '01/01',
      rawPayload,
      ...
      drafts: [
        {
          ...
          sync_lease_token: null,
          sync_lease_expires_at: null,
          position_snapshot_json: JSON.stringify(snapshot),
          label_id: null,
          product_results_json: null,
      rejections_json: null,
          position_detected: 0,
  ...
  it('replays position by capture sequence (ignores out-of-order product snapshots)', () => {
    const now = '2026-01-01T00:00:00.000Z';
    const session = {
      id: 'session-asp',
      inventory_id: 'inv-1',
    ...
    const photo = (seq: number) =>
      ({
        id: `session-asp:${seq}`,
        capture_session_id: 'session-asp',
        asset_id: String(seq),
        media_store_numeric_id: seq,
        uri: `file://${seq}.jpg`,
        display_name: `${seq}.jpg`,
        mime_type: 'image/jpeg',
        size: 10,
        width: 1,
        height: 1,
    ...
    const posSnap = (labelId: string) =>
      JSON.stringify({
        labelId,
        positionLabelId: labelId,
        displayName: labelId,
        rawPayload: labelId,
        sourcePayload: labelId,
        pallet: null,
        side: labelId.endsWith('-L') ? 'LEFT' : 'RIGHT',
        level: null,
        markerIndex: null,
        markerTotal: null,
      });
    ...
    const baseDraft = {
      ...
      candidate_count: 1,
      processing_ms: 10,
      comparison_status: 'PENDING' as const,
      compare_result: null,
      compared_at: null,
      prepared_asset_fingerprint: 'sha256:x',
      scan_owner: null,
    ...
    // Photo1=L marker; photos 2–3 products wrongly frozen as R (out-of-order scan);
    // photo4=R marker; photos 5–6 products under R.
    const drafts = [
      {
        ...baseDraft,
        id: 'd1',
        capture_photo_id: 'session-asp:1',
        client_file_id: 'cf-1',
        status: 'DETECTED_UNVERIFIED' as const,
        internal_code: null,
        quantity: null,
        quantity_status: 'NOT_APPLICABLE',
        error_code: 'POSITION_LABEL_DETECTED',
        position_snapshot_json: posSnap('ASP-A01-P03-L'),
        label_id: null,
        product_results_json: null,
        position_detected: 1,
      },
      {
        ...baseDraft,
        id: 'd2',
        capture_photo_id: 'session-asp:2',
        client_file_id: 'cf-2',
        internal_code: 'ASI-7K2M9Q',
        quantity: 24,
        quantity_status: 'PRESENT',
        error_code: null,
        position_snapshot_json: posSnap('ASP-A01-P04-R'),
        label_id: 'ASI-7K2M9Q',
        product_results_json: JSON.stringify([
          { labelId: 'ASI-7K2M9Q', internalCode: 'ASI-7K2M9Q', quantity: 24 },
        ]),
        position_detected: 0,
      },
      {
        ...baseDraft,
        id: 'd3',
        capture_photo_id: 'session-asp:3',
        client_file_id: 'cf-3',
        internal_code: 'ASI-4F8N3C',
        quantity: 12,
        quantity_status: 'PRESENT',
        error_code: null,
        position_snapshot_json: posSnap('ASP-A01-P04-R'),
        label_id: 'ASI-4F8N3C',
        product_results_json: JSON.stringify([
          { labelId: 'ASI-4F8N3C', internalCode: 'ASI-4F8N3C', quantity: 12 },
        ]),
        position_detected: 0,
      },
      {
        ...baseDraft,
        id: 'd4',
        capture_photo_id: 'session-asp:4',
        client_file_id: 'cf-4',
        status: 'DETECTED_UNVERIFIED' as const,
        internal_code: null,
        quantity: null,
      ...
      },
      {
        ...baseDraft,
        id: 'd5',
        capture_photo_id: 'session-asp:5',
        client_file_id: 'cf-5',
        internal_code: 'ASI-9T6R2V',
        quantity: 48,
        quantity_status: 'PRESENT',
        error_code: null,
        position_snapshot_json: posSnap('ASP-A01-P04-R'),
        label_id: 'ASI-9T6R2V',
        product_results_json: JSON.stringify([
          { labelId: 'ASI-9T6R2V', internalCode: 'ASI-9T6R2V', quantity: 48 },
        ]),
        position_detected: 0,
      },
      {
        ...baseDraft,
        id: 'd6',
        capture_photo_id: 'session-asp:6',
        client_file_id: 'cf-6',
        internal_code: 'ASI-2W5H8D',
        quantity: 6,
        quantity_status: 'PRESENT',
        error_code: null,
        position_snapshot_json: posSnap('ASP-A01-P04-R'),
        label_id: 'ASI-2W5H8D',
        product_results_json: JSON.stringify([
          { labelId: 'ASI-2W5H8D', internalCode: 'ASI-2W5H8D', quantity: 6 },
        ]),
        position_detected: 0,
      },
    ];
    ...
    const rows = buildLocalCsvRows({
      ...
      companyId: null,
      clientId: 'client-1',
      exportId: 'export-asp-seq',
      exportedAt: now,
    });

    expect(rows.map((r) => r.position_label_id)).toEqual([
      'ASP-A01-P03-L',
      'ASP-A01-P03-L',
      'ASP-A01-P03-L',
      'ASP-A01-P04-R',
    ...
    expect(rows[0]?.source).toBe('LOCAL_POSITION_LABEL');
    expect(rows[1]?.internal_code).toBe('ASI-7K2M9Q');
    expect(rows[3]?.source).toBe('LOCAL_POSITION_LABEL');
    expect(rows[4]?.internal_code).toBe('ASI-9T6R2V');
  });
});
