  ...
  const repo = {
    ...
    async upsertDraft(input: {
      ...
      const row: LocalDetectionDraftRow = {
        ...
        scan_generation: gen,
        sync_status: existing?.sync_status ?? 'NOT_READY',
        sync_attempt_count: existing?.sync_attempt_count ?? 0,
        sync_next_retry_at: existing?.sync_next_retry_at ?? null,
        sync_last_error_code: existing?.sync_last_error_code ?? null,
        server_preliminary_id: existing?.server_preliminary_id ?? null,
